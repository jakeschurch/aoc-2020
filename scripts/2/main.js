...
console.log(
  list
    .map((line) => {
      const [_, min, max, targetChar, password] = line.match(re);
      const charCount = [...password].filter((char) => char === targetChar)
        .length;
...
console.log(
  list
    .map((line) => {
      const [_, posA, posB, targetChar, password] = line.match(re);
      const posCheckA = [...password][posA - 1] === targetChar
      const posCheckB = [...password][posB - 1] === targetChar
      const posCheck = (posCheckA ? !posCheckB : posCheckB)
